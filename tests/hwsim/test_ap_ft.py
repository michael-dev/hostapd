...
def run_roams(dev, apdev, hapd0, hapd1, ssid, passphrase, over_ds=False, sae=False, eap=False, fail_test=False, roams=1, eap_identity="gpsk user", conndev=False):
...
def test_ap_ft_vlan_2(dev, apdev):
    """WPA2-PSK-FT AP w VLAN and dest-AP does not have VLAN info locally"""
    ssid = "test-ft"
    passphrase="12345678"
...
def test_ap_ft_many_vlan(dev, apdev):
    ...
    run_roams(dev[0], apdev, hapd0, hapd1, ssid, passphrase, roams=50, conndev="brvlan1")
...
def test_ap_ft_vlan_over_ds(dev, apdev):
    ...
    run_roams(dev[0], apdev, hapd0, hapd1, ssid, passphrase, over_ds=True, conndev="brvlan1")
    check_mib(dev[0], [ ("dot11RSNAAuthenticationSuiteRequested", "00-0f-ac-4"),
                        ("dot11RSNAAuthenticationSuiteSelected", "00-0f-ac-4") ])
...
def test_ap_ft_over_ds_pull_vlan(dev, apdev):
    """WPA2-PSK-FT AP over DS (pull PMK) w VLAN"""
    ssid = "test-ft"
    passphrase="12345678"
...
def test_ap_ft_eap_over_ds(dev, apdev):
    """WPA2-EAP-FT AP w over_ds"""
    generic_ap_ft_eap(dev, apdev, vlan=False, over_ds=True)
...
def generic_ap_ft_eap_pull(dev, apdev, vlan):
    ...
    run_roams(dev[0], apdev, hapd, hapd1, ssid, passphrase, eap=True, eap_identity=identity, conndev=conndev)

def test_ap_ft_eap_pull(dev, apdev):
    generic_ap_ft_eap_pull(dev, apdev, vlan=False)
