				     ...
				     struct wpa_init_params *params)
{
	...
	drv->ifidx = if_nametoindex(drv->ifname);
	if (drv->ifidx < 0) {
				     ...
				     int setup_ap)
{
	...
	if (if_nametoindex(ifname) > 0) {
		...
		wiredng_remove_iface(drv, if_nametoindex(ifname));
	}
	...
	if (!ret) {
		...
		if (newVlanIfIdx) {
			...
			drv->vlanInterfaceIdx[drv->numVlanInterfaces] = if_nametoindex(ifname);
				       ...
				       const char *ifname, int vlan_id)
{
	...
	macvlan_interface_change_mac(if_nametoindex(ifname), 1, addr);
	...
	drv->sta[i].ifidx = if_nametoindex(ifname);
