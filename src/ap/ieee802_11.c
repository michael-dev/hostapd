...
static void ieee802_11_radius_cb(struct hostapd_data *hapd, const u8 *buf,
...
ieee802_11_allowed_address(struct hostapd_data *hapd, const u8 *addr,
			   const u8 *msg, size_t len, struct radius_sta *info)
{
	int res;

	res = hostapd_allowed_address(hapd, addr, msg, len, info,
				      &ieee802_11_radius_cb);
			...
			const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	res = ieee802_11_allowed_address(
		hapd, mgmt->sa, (const u8 *) mgmt, len, &info);
	if (res == HOSTAPD_ACL_REJECT) {
		resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
		goto fail;
	}
	...
	res = ieee802_11_set_radius_info(hapd, sta, res, &info);
	if (res) {
		resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
		goto fail;
	}
