			      ...
			      u8 **plain, size_t *plain_size)
{
	...
	*plain = NULL;
	...
	len = data_len - AES_BLOCK_SIZE;

	/* length a multiple of blocksize? */
	if (len % AES_BLOCK_SIZE != 0)
		goto err;

	*plain = os_zalloc(data_len);
	if (!*plain)
		goto err;

	if (aes_siv_decrypt(key, key_len, data, data_len, 0,
			    NULL, NULL, *plain) < 0)
		goto err;
...
{
	*packet_len = plain_len + AES_BLOCK_SIZE;
	*packet = os_zalloc(*packet_len);
	...
	if (aes_siv_encrypt(key, key_len, plain,
			    plain_len, 0, NULL, NULL, *packet) < 0)
		goto err;
			    ...
			    u8 **packet, size_t *packet_len)
{
	...
	if (wpa_ft_rrb_lin(tlvs1, tlvs2, vlan, &plain, &plain_len) < 0)
		goto out;

	if (wpa_ft_rrb_encrypt(key, key_len, plain, plain_len, packet,
			       packet_len) < 0)
		goto out;
...
#define RRB_GET_OPTIONAL(type, field, txt, checklength) do { \
	if (wpa_ft_rrb_get_tlv(plain, plain_len, type, \
				&f_##field##_len, &f_##field) < 0 || \
	    (checklength > 0 && ((size_t) checklength) != f_##field##_len)) { \
		wpa_printf(MSG_DEBUG, "FT: Missing " #field " in PMK-R1 " \
			      ...
			      const u8 *pmk_r0_name)
{
	...
	if (wpa_ft_rrb_build(r0kh->key, sizeof(r0kh->key), req_tlv, NULL, NULL,
			     &packet, &packet_len) < 0) {
		return -1;
	}
			       ...
			       u8 **packet, size_t *packet_len)
{
	...
	if (!pmk_r0)
		return wpa_ft_rrb_build(key, key_len, tlvs, NULL, NULL, packet,
					packet_len);
	...
	ret = wpa_ft_rrb_build(key, key_len, tlvs, sess_tlv, pmk_r0->vlan,
			       packet, packet_len);
			      ...
			      const u8 *data, size_t data_len)
{
	...
	if (wpa_ft_rrb_decrypt(r1kh->key, sizeof(r1kh->key), data, data_len,
			       &plain, &plain_len) < 0) {
	...
	ret = wpa_ft_rrb_build_r0(r1kh->key, sizeof(r1kh->key), resp_tlv, r0,
			      ...
			      const u8 *data, size_t data_len)
{
	...
	if (wpa_ft_rrb_decrypt(r0kh->key, sizeof(r0kh->key), data, data_len,
			       &plain, &plain_len) < 0) {
		wpa_printf(MSG_DEBUG, "FT: Failed to decrypt PMK-R1 pull "
				   ...
				   const u8 *s1kh_id)
{
	...
	if (wpa_ft_rrb_build_r0(r1kh->key, sizeof(r1kh->key), push_tlv, pmk_r0,
