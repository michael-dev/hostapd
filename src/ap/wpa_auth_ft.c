...
static int wpa_ft_send_rrb_auth_resp(struct wpa_state_machine *sm,
				     ...
				     size_t resp_ies_len);
static void ft_pull_resp_cb_finish(struct wpa_state_machine *sm);
...
#define RRB_GET_OPTIONAL_SRC(srcfield, type, field, txt, checklength) do { \
	if (wpa_ft_rrb_get_tlv(srcfield, srcfield##_len, type, \
		...
		wpa_printf(MSG_DEBUG, "FT: Missing optional " #field " in " \
			   "PMK-R1 %s from " MACSTR, txt, MAC2STR(src_addr)); \
		f_##field##_len = 0; \
		f_##field = NULL; \
	} \
} while (0)
			      ...
			      const u8 *pmk_r0_name)
{
	...
	if (wpa_ft_rrb_build(r0kh->key, sizeof(r0kh->key), req_enc, NULL,
			     req_auth, NULL, sm->wpa_auth->addr,
			      ...
			      int noDefer)
{
	...
	RRB_GET_AUTH(FT_RRB_R0KH_ID, r0kh_id, "pull request", -1);
	...
	if (r1kh != r1kh_wildcard &&
	    wpa_ft_rrb_seq_chk(wpa_auth, &r1kh->seq, src_addr, &rkh_id_local,
			       &rkh_id_remote, enc, enc_len, auth, auth_len,
	...
	if (r1kh == NULL) /* wpa_ft_rrb_add_r1kh failed */
		r1kh = r1kh_wildcard;

	if (r1kh != r1kh_wildcard)
		wpa_ft_rrb_seq_accept(wpa_auth, &r1kh->seq, src_addr, auth,
				      auth_len, "pull request");
	...
	ret = wpa_ft_rrb_build_r0(r1kh->key, sizeof(r1kh->key), resp, r0,
				  f_r1kh_id, f_s1kh_id, resp_auth,
				  wpa_auth->addr, FT_PACKET_R0KH_R1KH_RESP,
				  &packet, &packet_len);
				      ...
				      int noDefer))
{
	...
	if (r0kh == NULL) {
		wpa_printf(MSG_DEBUG, "FT: PMR-R1 %s No matching R0KH address "
			   "found for PMK-R0 pull response source address "
			   MACSTR, msgtype, MAC2STR(src_addr));
		return -1;
	}
	RRB_GET_AUTH(FT_RRB_R0KH_ID, r0kh_id, msgtype, -1);
	if (r0kh != r0kh_wildcard && (r0kh->id_len != f_r0kh_id_len ||
	...
	if (r0kh != r0kh_wildcard &&
	    wpa_ft_rrb_seq_chk(wpa_auth, &r0kh->seq, src_addr, &rkh_id_local,
			       &rkh_id_remote, enc, enc_len, auth, auth_len,
	...
	if (r0kh == r0kh_wildcard && wpa_auth->conf.rkh_pos_timeout)
		r0kh = wpa_ft_rrb_add_r0kh(wpa_auth, r0kh_wildcard, src_addr,
					   f_r0kh_id, f_r0kh_id_len,
					   wpa_auth->conf.rkh_pos_timeout);
	if (r0kh == NULL) /* wpa_ft_rrb_add_r0kh failed */
		r0kh = r0kh_wildcard;
	...
	if (f_identity) {
		os_snprintf(buf, sizeof(buf), "FT: PMK-R1 %s - Identity",
			    msgtype);
		wpa_hexdump(MSG_DEBUG, buf, f_identity, f_identity_len);
	}

	RRB_GET_OPTIONAL(FT_RRB_RADIUS_CUI, radius_cui, msgtype, -1);
	if (f_radius_cui) {
		os_snprintf(buf, sizeof(buf), "FT: PMK-R1 %s - Cui",
			    msgtype);
			      ...
			      int noDefer)
{
	...
	RRB_GET_AUTH(FT_RRB_NONCE, nonce, "pull response", FT_RRB_NONCE_LEN);
	wpa_hexdump(MSG_DEBUG, "FT: PMK-R1 pull - nonce", f_nonce, f_nonce_len);

	os_memset(&ctx, 0, sizeof(ctx));
	ctx.nonce = f_nonce;
	if (!wpa_auth_for_each_sta(wpa_auth, ft_pull_resp_cb, &ctx))
			      ...
			      int noDefer)
{
	...
	RRB_GET_AUTH(FT_RRB_TIMESTAMP, timestamp, "push", sizeof(le32));
	os_get_time(&now);
			     ...
			     struct tlv_list *rkh_id_remote)
{
	...
	u8 *plain = NULL;
	size_t plain_len = 0;
	struct ft_remote_r0kh *r0kh_wildcard = NULL;
	struct ft_remote_r1kh *r1kh_wildcard = NULL;
