				   ...
				   struct vlan_description *vlan)
{
	...
	while (left >= sizeof(*f)) {
		...
		if (type == FT_RRB_VLAN_UNTAGGED &&
		    len != sizeof(le16)) {
			wpa_printf(MSG_DEBUG, "FT: RRB VLAN_UNTAGGED invalid "
				   "length");
			return -1;
		}
		...
		if (type == FT_RRB_VLAN_TAGGED &&
		    len % sizeof(le16) != 0) {
			wpa_printf(MSG_DEBUG, "FT: RRB VLAN_TAGGED invalid "
				   "length");
			return -1;
		}
		...
		while (len >= sizeof(le16)) {
			...
			if (vlan_id <= 0 || vlan_id > MAX_VLAN_ID) {
				wpa_printf(MSG_DEBUG, "FT: RRB VLAN ID invalid"
				   " %d", vlan_id);
				continue;
			}
			      ...
			      const u8 *src_addr, const u8 type, u8 *enc)
{
	...
	if (aes_siv_encrypt(key, key_len, plain,
			    plain_len, 4, ad, ad_len, enc) < 0) {
			    ...
			    u8 **packet, size_t *packet_len)
{
	...
	if (wpa_ft_rrb_lin(tlvs_enc0, tlvs_enc1, vlan, key ? 1 : 0,
			   &plain, &plain_len) < 0)
		goto out;

	if (wpa_ft_rrb_lin(tlvs_auth, NULL, NULL, 0, &auth, &auth_len) < 0)
		goto out;
	...
	*packet = os_zalloc(*packet_len);
	...
	} else if (wpa_ft_rrb_encrypt(key, key_len, plain, plain_len, auth,
...
#define RRB_GET(type, field, txt, checklength) RRB_GET_SRC(plain, type, field, txt, checklength)
#define RRB_GET_AUTH(type, field, txt, checklength) RRB_GET_SRC(auth, type, field, txt, checklength)
...
#define RRB_GET_OPTIONAL(type, field, txt, checklength) RRB_GET_OPTIONAL_SRC(plain, type, field, txt, checklength)
#define RRB_GET_OPTIONAL_AUTH(type, field, txt, checklength) RRB_GET_OPTIONAL_SRC(auth, type, field, txt, checklength)
	       ...
	       void *cb_ctx, int cb_ctx_len)
{
	...
	return wpa_auth->cb->add_sta(wpa_auth->cb_ctx, sta_addr, sm, cb,
				     cb_ctx, cb_ctx_len);
}
		    ...
		    const u8 *identity, size_t identity_len)
{
	if (wpa_auth->cb->set_identity == NULL)
		return;
	wpa_auth->cb->set_identity(wpa_auth->cb_ctx, sta_addr, identity,
				   identity_len);
}
		      ...
		      const u8 *radius_cui, size_t radius_cui_len)
{
	if (wpa_auth->cb->set_radius_cui == NULL)
		return;
	wpa_auth->cb->set_radius_cui(wpa_auth->cb_ctx, sta_addr, radius_cui,
				     radius_cui_len);
}
...
static void wpa_ft_rrb_seq_flush(struct wpa_authenticator *wpa_auth,
	...
	dl_list_for_each_safe(item, n, &rkh_seq->queue,
...
static void wpa_ft_rrb_seq_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_ft_rrb_seq_flush(NULL, rkh_seq, 0);
}
			     ...
			     int noDefer))
{
	...
	const int first = dl_list_empty(&rkh_seq->queue);

	if (first && random_get_bytes(rkh_seq->nonce, FT_RRB_NONCE_LEN) < 0) {
		wpa_printf(MSG_DEBUG, "FT: Seq num nonce: out of random bytes");
		return -1;
	}

	if (first && os_get_reltime(&rkh_seq->nonce_ts) < 0) {
		wpa_printf(MSG_DEBUG, "FT: Seq num nonce: failed to get"
				   " timestamp");
		return -1;
	}

	if (dl_list_len(&rkh_seq->queue) >= ftRRBmaxQueueLen) {
	...
	dl_list_add(&rkh_seq->queue, &item->list);
		   ...
		   const char *msgtype, int noDefer)
{
	...
	if (msg_dom != rkh_seq->dom)
		/* remote might have rebootet */
		goto defer;

	if (os_get_reltime(&now) == 0) {
		os_reltime_sub(&now, &rkh_seq->time_offset, &now_remote);
...
static int wpa_ft_new_seq(struct wpa_authenticator *wpa_auth,
			       ...
			       const u8 *radius_cui, size_t radius_cui_len)
{
	...
	if (identity) {
		r0->identity = os_zalloc(identity_len);
		if (r0->identity) {
	...
	if (radius_cui) {
		r0->radius_cui = os_zalloc(radius_cui_len);
		if (r0->radius_cui) {
			       ...
			       const u8 *radius_cui, u8 radius_cui_len)
{
	...
	if (identity) {
		r1->identity = os_zalloc(identity_len);
		if (r1->identity) {
	...
	if (radius_cui) {
		r1->radius_cui = os_zalloc(radius_cui_len);
		if (r1->radius_cui) {
...
wpa_ft_rrb_add_r0kh(struct wpa_authenticator *wpa_auth,
		    struct ft_remote_r0kh *r0kh_wildcard,
		    const u8 *src_addr, const u8 *r0kh_id, size_t id_len, int timeout)
{
			      ...
			      const u8 *pmk_r0_name)
{
	...
	if (wpa_ft_new_seq(sm->wpa_auth, &f_seq) < 0) {
	...
	wpa_ft_rrb_oui_send(sm->wpa_auth, addr, FT_PACKET_R0KH_R1KH_PULL,
			    packet, packet_len);
			      ...
			      int noDefer)
{
	...
	wpa_ft_rrb_lookup_r1kh(wpa_auth, src_addr, f_r1kh_id,
			       &r1kh, &r1kh_wildcard);
	if (r1kh) {
		key = r1kh->key;
	...
	if (!r1kh && wpa_auth->conf.rkh_pos_timeout) {
	...
	if (r1kh && seq_ret == FT_RRB_SEQ_DEFER) {
		wpa_ft_rrb_seq_req(wpa_auth, &r1kh->seq, src_addr,
				   enc, enc_len, auth, auth_len,
				   msgtype, key, key_len, &wpa_ft_rrb_rx_pull);
		goto out;
	}

	if (r1kh)
		wpa_ft_rrb_seq_accept(wpa_auth, &r1kh->seq, src_addr, auth,
				      auth_len, msgtype);

	RRB_GET(FT_RRB_PMK_R0_NAME, pmk_r0_name, msgtype, WPA_PMK_NAME_LEN);
	wpa_hexdump(MSG_DEBUG, "FT: PMKR0Name", f_pmk_r0_name, f_pmk_r0_name_len);
				      ...
				      int noDefer))
{
	...
	if (!r0kh && wpa_auth->conf.rkh_pos_timeout) {
	...
	if (r0kh && seq_ret == FT_RRB_SEQ_DEFER) {
		wpa_ft_rrb_seq_req(wpa_auth, &r0kh->seq, src_addr,
				   enc, enc_len, auth, auth_len,
	...
	if (r0kh)
		wpa_ft_rrb_seq_accept(wpa_auth, &r0kh->seq, src_addr, auth,
				      auth_len, msgtype);
	...
	if (f_identity)
		wpa_hexdump(MSG_DEBUG, "FT: Identity", f_identity, f_identity_len);

	RRB_GET_OPTIONAL(FT_RRB_RADIUS_CUI, radius_cui, msgtype, -1);
	if (f_radius_cui)
		wpa_hexdump(MSG_DEBUG, "FT: CUI", f_radius_cui, f_radius_cui_len);
			      ...
			      int noDefer)
{
	...
	ctx.s1kh_id = s1kh_id;
	if (!ret && wpa_auth_for_each_sta(wpa_auth, ft_get_sta_cb, &ctx)) {
		wpa_printf(MSG_DEBUG, "FT: Response to a pending pull request for "
			   MACSTR " - process from timeout", MAC2STR(ctx.sm->addr));
			     ...
			     u8 **key, size_t *key_len)
{
	...
	if (!to_r0kh) {
		wpa_ft_rrb_lookup_r0kh(wpa_auth, src_addr, f_r0kh_id,
				       f_r0kh_id_len, r0kh, &r0kh_wildcard);
		if (*r0kh) {
			*key = (*r0kh)->key;
			*key_len = sizeof((*r0kh)->key);
		} else if (r0kh_wildcard) {
				  ...
				  int noDefer)
{
	...
	if (r1kh)
		rkh_seq = &r1kh->seq;
	if (!rkh_seq) /* e.g. with wildcard */
		goto out;

	if (os_get_reltime(&now) < 0 ||
	    !os_reltime_initialized(&rkh_seq->nonce_ts) ||
	    os_memcmp_const(f_nonce, rkh_seq->nonce, FT_RRB_NONCE_LEN) != 0 ||
	    os_reltime_expired(&now, &rkh_seq->nonce_ts, ftRRBseqTimeout)) {
		       ...
		       size_t data_len)
{
	...
	const u16 *head = (u16 *) data;
	if (data_len < sizeof(*head)) {
		wpa_printf(MSG_DEBUG, "FT: RRB-OUI frame too short");
		return;
	}

	size_t alen = le_to_host16(*head);
	if (data_len < sizeof(*head) + alen) {
		wpa_printf(MSG_DEBUG, "FT: RRB-OUI frame too short");
		return;
	}

	const u8 *auth = data + sizeof(*head);
	const u8 *enc = data + sizeof(*head) + alen;
	size_t elen = data_len - sizeof(*head) - alen;
	...
	case FT_PACKET_R0KH_R1KH_SEQ_RESP:
		wpa_ft_rrb_rx_seq_resp(wpa_auth, src_addr, enc, elen, auth, alen,
				       noDefer);
		break;
	}
}
