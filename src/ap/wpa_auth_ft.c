		   ...
		   const char *msgtype, int noDefer)
{
	...
	if (os_get_reltime(&now) == 0) {
		...
		msg_ts_off = msg_ts - (msg_ts_now_remote - ftRRBseqTimeout);
...
void wpa_ft_deinit_seq(struct wpa_authenticator *wpa_auth)
{
	struct ft_remote_r0kh *r0kh;
	struct ft_remote_r1kh *r1kh;
...
void wpa_ft_deinit_rkh_tmp(struct wpa_authenticator *wpa_auth)
{
			      ...
			      int noDefer)
{
	...
	resp_auth[4].type = FT_RRB_MOBILITY_DOMAIN;
	resp_auth[4].len = MOBILITY_DOMAIN_ID_LEN;
	resp_auth[4].data = mdid;
	resp_auth[5].type = FT_RRB_LAST_EMPTY;
	resp_auth[5].len = 0;
	resp_auth[5].data = NULL;
